...
p1 <- DimPlot(killi_young_integrated, reduction = "tsne", pt.size = 1.8, label=T) +NoLegend()
p2 <- DimPlot(killi_young_integrated, reduction = "tsne", pt.size = 0.8, split.by="sample",label.size=4)
p1+p2
...
dittoScatterPlot(killi_young_integrated, 
                 ...
                 assay.x = "RNA", assay.y = "RNA", color.var = "CellType" 
)
scale_color_manual(values = killi_young_integrated$CellType) +
...
table(Idents(killi_young_integrated))

##Plot joint density of PCNA and glul genes; glial/nonglial signature##
Plot_Density_Joint_Only(killi_young_integrated, c("PCNA","GLUL (2 OF 2)"), reduction="tsne")
