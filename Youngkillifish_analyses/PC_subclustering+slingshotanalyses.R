...
table(Idents(PCs_integrated))
par(mfrow=c(2,2))
p1 <-RidgePlot(PCs_integrated, cols=c("lightsteelblue","gray","#FAB693", "#FA340D", "powderblue","darkcyan"), idents = c("NE-RG3","NGP.1","NGP.2","Astro-RG1","Astro-RG2","EPD-RG4"), features=c("EPD"))
p2 <- RidgePlot(PCs_integrated, cols=c("lightsteelblue","gray","#FAB693", "#FA340D", "powderblue","darkcyan"), idents = c("NE-RG3","NGP.1","NGP.2","Astro-RG1","Astro-RG2","EPD-RG4"), features=c("ZIC2"))
p3 <- RidgePlot(PCs_integrated, cols=c("lightsteelblue","gray","#FAB693", "#FA340D", "powderblue","darkcyan"), idents = c("NE-RG3","NGP.1","NGP.2","Astro-RG1","Astro-RG2","EPD-RG4"), features=c("SOX2"))
p4 <- RidgePlot(PCs_integrated, cols=c("lightsteelblue","gray","#FAB693", "#FA340D", "powderblue","darkcyan"), idents = c("NE-RG3","NGP.1","NGP.2","Astro-RG1","Astro-RG2","EPD-RG4"), features=c("PCNA"))
par(mfrow = c(1, 1))
...
aggregate_pcs <- AggregateExpression(PCs_integrated, group.by = c("seurat_clusters", "condition"), return.seurat = TRUE)
head(aggregate_pcs)
genes.to.label = c("ISG15", "LY6E", "IFI6", "ISG20", "MX1", "IFIT2", "IFIT1", "CXCL10", "CCL8")
DefaultAssay(PCs_integrated) <- "integrated"
p1 <- CellScatter(aggregate_pcs, "NE-RG3_AgedK", "NE-RG3_YoungK", highlight = genes.to.label)
...
PCs_integrated_new <- subset(PCs_integrated, idents=c("NE-RG3","NGP.1","NGP.2","Astro-RG1","Astro-RG2"))
DotPlot(PCs_integrated, features= c("SOX2","PCNA","HMGB2A","STMN1A","SLC1A2B","CX43"),split.by = "condition", cols=c("green","purple")) + RotatedAxis()+ coord_flip()
...
VlnPlot(PCs_integrated, idents = c("NGP.1","NGP.2"),cols=c("#FAB693","gray"),flip=T,fill.by="ident",features=c("MDK","SOX2","PCNA","MKI67","NEUROG1","CDK1","STMN1A","HMGB2A","KIAA0101","MSI1","MCM2","MCM4","MCM5"),stack=T,pt.size=3)
?VlnPlot  
...
Idents(PCs_integrated) <- PCs_integrated$CellType
...
FeaturePlot(PCs_lineages_Yall, features=c("PCNA","MKI67","ELAVL3","HMGB2B","MARCKS","ASCL1B"),label=T , reduction="tsne")
FeaturePlot(PCs_lineages_Yall, features=c("PCNA","GLUL (2 OF 2)"),label=T , reduction="tsne")
...
plotGeneCount(curves_PC, counts=scale_data_Y_allS, gene="HES5 (3 OF 9)")
