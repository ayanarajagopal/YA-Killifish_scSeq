...
DotPlot(NNs_newanno_allS,features= c("SLC17A7","SLC17A6B","GAD2","GAD1B","MAP2","SV2A","MEX3A","MIBP2","OLA.22466","STMN1B","BHLHE22","EOMESA","NEUROD2","ELAVL3","DLX1","DLX2","PCNA","MDK"), cols=c("green","purple"),cluster.idents=T)+ RotatedAxis()
...
FeaturePlot(NNs_newanno_allS, features=c("EOMESA","DLX1"), order=T, reduction="tsne", label=T, pt.size = 1.0)
...
names(NCs_res_0.9_mar) <- levels(NNs_newanno_allS)
NNs_newanno_allS <- RenameIdents(NNs_newanno_allS, NCs_res_0.9_mar)
table(Idents(NNs_newanno_allS))
